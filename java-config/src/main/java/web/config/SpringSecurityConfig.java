...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.authorizeRequests().accessDecisionManager(accessDecisionManager())
				.expressionHandler(webSecurityExpressionHandler()).antMatchers("/**/*.do*").hasRole("USER")
		...
		// 自定义登录页面
		httpSecurity.csrf().disable().formLogin().loginPage("/login").failureUrl("/login?error=1")
		...
		// session管理
		httpSecurity.sessionManagement().sessionFixation().changeSessionId().maximumSessions(1).expiredUrl("/");

		// RemeberMe
		httpSecurity.rememberMe().key("webmvc#FD637E6D9C0F1A5A67082AF56CE32485");
