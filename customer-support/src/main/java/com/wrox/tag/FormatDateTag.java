...
public class FormatDateTag extends TagSupport {
	...
	static {
		try {
			GET_LOCALE_METHOD = SetLocaleSupport.class.getDeclaredMethod("getFormattingLocale", PageContext.class, Tag.class,
					boolean.class, boolean.class);
			GET_LOCALE_METHOD.setAccessible(true);
			GET_TIME_ZONE_METHOD = TimeZoneSupport.class.getDeclaredMethod("getTimeZone", PageContext.class, Tag.class);
	...
	private String formatDate(TemporalAccessor value, Locale locale) {
		...
		if (this.type == Type.DATE || this.type == Type.BOTH)
			dateFormat = DateTimeFormatter.ofLocalizedDate(this.dateStyle.getFormatStyle()).withLocale(locale).withZone(zoneId)::format;
		if (this.type == Type.TIME || this.type == Type.BOTH)
			timeFormat = DateTimeFormatter.ofLocalizedTime(this.timeStyle.getFormatStyle()).withLocale(locale).withZone(zoneId)::format;
	...
	private static enum Style {
		DEFAULT(DateFormat.MEDIUM, FormatStyle.MEDIUM), SHORT(DateFormat.SHORT, FormatStyle.SHORT), MEDIUM(DateFormat.MEDIUM,
				FormatStyle.MEDIUM), LONG(DateFormat.LONG, FormatStyle.LONG), FULL(DateFormat.FULL, FormatStyle.FULL);
